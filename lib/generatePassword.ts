...
const characterPool = ({ includeUppercase, includeLowercase, includeNumbers, includeSpecial, easyRead }: GeneratePassword) => {
...
export const generatePassword = ({ includeUppercase, includeLowercase, includeNumbers, includeSpecial, easyRead, length }: GeneratePassword): {
    password: string
    score: keyof typeof PasswordStrength
} => {
    const pool = characterPool({includeUppercase, includeLowercase, includeNumbers, includeSpecial, easyRead})
    if (pool === "") {
        return {
            password: "",
            score: "Weak"
