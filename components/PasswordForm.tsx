...
import { passwordReducer, initialState, checkBoxItems } from '../lib/passwordReducer';
...
export function PasswordForm() {
    ...
    const characterPool = useMemo(() => {
        ...
        return pool;
    }, [state.includeUppercase, state.includeLowercase, state.includeNumbers, state.includeSpecial, state.easyRead]);
    ...
    const generateScore = () => {
        const trueCount = [state.includeUppercase, state.includeLowercase, state.includeNumbers, state.includeSpecial].filter(Boolean).length;
        switch (trueCount) {
            case 4:
                return 100
            case 3:
    ...
    const handleCheckboxChange = (type: keyof CheckBoxSelectorState) => {
        ...
        const checkedItems = Object.keys(updatedState)
            .filter(key => (updatedState[key as keyof typeof updatedState] as boolean) && key !== "length" && key !== "password");
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12 w-full sm:w-[40%] lg:w-[30%] 2xl:w-[20%]">
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            {score && <FormDescription>
                                <Progress value={score} className={`${score < 63 ? 'bg-red-500' : score < 99 ? 'bg-yellow-500' : 'bg-green-500'}`} data-test-id="password-score-progress" />
                                <p data-test-id="password-score">{score < 63 ? 'Weak' : score < 99 ? 'Medium' : 'Strong'}</p>
                            </FormDescription>}
                            <FormMessage />
                        </FormItem>
                    )}
                ...
                <div className="flex flex-col gap-2">
                    <div className='w-full flex gap-2'>
                        ...
                        <input
                            ...
                            onChange={(e) => dispatch({ type: 'SET_LENGTH', payload: Number(e.target.value) })}
                            min="6"
                            max="30"
                        />
                    </div>
                    ...
                    {checkBoxItems.map(item => {
                        return <div key={item.selector} className="flex items-center space-x-2">
                            <Checkbox dataTestId={item.selector} name={item.selector} checked={state[item.selector]} onCheckedChange={() => handleCheckboxChange(item.selector)} />
                            <label
                                htmlFor="terms2"
                ...
                <Button type="submit" data-test-id="generate-password-btn" onClick={generatePassword}>Generate Password</Button>
            </form>
        </Form>
    );
}
